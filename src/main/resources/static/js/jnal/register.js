...
function renderJnal() {
    ...
    d6_removeMethodBtn.setAttribute("onclick", "removeMethod(0)");
    ...
    d4_saveBtn.setAttribute("onclick", "registerJnal()");
...
/**
 * JNAL Event4) 새 운동일지 등록
 * */
function registerJnal() {
    ...
    methods.forEach(function(method){
        ...
        // 운동종목이 비었을경우 운동종목은 저장하지 않고 패스한다.
        if (methodId === "null") {
            return false ;
        }
    ...
    $.ajax({
        ...
        ,
        error: function (request, error) {
            console.log("운동일지 생성 중 오류가 발생하였습니다..\n");
            alert("code:"+request.status+"\n"
                +"message:"+request.responseText+"\n"
                +"error:"+error)
        }
