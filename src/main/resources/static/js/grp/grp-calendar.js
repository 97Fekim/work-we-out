...
function renderCalendar(date) {
    ...
    $.ajax({
        ...
        success: function (oneMonthGrpCalendarDTO) {
            console.log("Calendar Load Success");
            renderCalendarContents(oneMonthGrpCalendarDTO);
        },
        error: function (request, error) {
 ...
 * */
function renderCalendarContents(oneMonthGrpCalendarDTO) {

    console.log(oneMonthGrpCalendarDTO);
    ...
    oneMonthGrpCalendarDTO.oneDayGrpCalendarDTOList.forEach(function(oneDayCalendarDTO, idx) {
        ...
        var d3_membersComponent = document.createElement("div");

        // Depth-3
        d3_membersComponent.setAttribute("style", "padding-left:3px;");
        oneDayCalendarDTO.memberGrpDTOList.forEach(function(memberDTO) {
            // Depth-4
            var d4_mbrItem = document.createElement("div");
            d4_mbrItem.setAttribute("class", "flex items-center");
            d4_mbrItem.setAttribute("style", "padding-left:3px; width:20%");

            // Depth-5
            var d5_mbrImg = document.createElement("img");
            d5_mbrImg.setAttribute("src", memberDTO.profImgPath === "EMPTY" ? "https://placehold.co/30" : "../img/" + memberDTO.profImgPath);
            d5_mbrImg.setAttribute("alt", "");
            d5_mbrImg.setAttribute("class", "rounded-full mr-1");
            var d5_mbrNm = document.createElement("span");
            d5_mbrNm.setAttribute("class", "text-xs");
            d5_mbrNm.setAttribute("style", "white-space:nowrap");
            d5_mbrNm.innerText = memberDTO.length > 4 ? memberDTO.mbrNm.substring(0,4) + ".." : memberDTO.mbrNm;

            d4_mbrItem.appendChild(d5_mbrImg);
            d4_mbrItem.appendChild(d5_mbrNm);

            d3_membersComponent.appendChild(d4_mbrItem);
        });

        // MERGE
        d2_dayComponent.appendChild(d3_ddComponent);
        d2_dayComponent.appendChild(d3_membersComponent);
